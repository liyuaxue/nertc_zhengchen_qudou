...
std::string Ml307Board::GetIccid() const {
    if (modem_ == nullptr) {
        return "";
    }
    ESP_LOGI(TAG, "Get4G---Iccid: %s", modem_->GetIccid().c_str());
    return modem_->GetIccid();
}
...
void Ml307Board::StartNetwork() {
    ...
    modem_->OnNetworkStateChanged([&application](bool network_ready) {
        if (network_ready) {
