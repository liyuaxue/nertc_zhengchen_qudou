...
LvglDisplay::LvglDisplay() {
    ...
    esp_timer_create_args_t notification_timer_args = {
        .callback = [](void *arg) {
            ...
            lv_async_call([](void* data) {
                auto* display = static_cast<LvglDisplay*>(data);
                if (display == nullptr) {
                    return;
                }
    ...
    // 低电量弹窗定时器（约 10 秒后自动隐藏）
    esp_timer_create_args_t low_battery_timer_args = {
        .callback = [](void *arg) {
            auto* display = static_cast<LvglDisplay*>(arg);
            if (display == nullptr) {
                return;
        ...
        .arg = this,
        .dispatch_method = ESP_TIMER_TASK,
        .name = "low_battery_timer",
        .skip_unhandled_events = true,
    };
    ESP_ERROR_CHECK(esp_timer_create(&low_battery_timer_args, &low_battery_timer_));

    // Volume timer (hide volume bar after 3 seconds)
    esp_timer_create_args_t volume_timer_args = {
        .callback = [](void *arg) {
            ...
            lv_async_call([](void* data) {
                ...
                DisplayLockGuard lock(display);
                if (display->volume_bar_container_) {
                    lv_obj_add_flag(display->volume_bar_container_, LV_OBJ_FLAG_HIDDEN);
                }
            }, display);
        },
        .arg = this,
...
LvglDisplay::~LvglDisplay() {
    ...
    if (low_battery_timer_ != nullptr) {
        esp_timer_stop(low_battery_timer_);
        esp_timer_delete(low_battery_timer_);
    }

    if (volume_timer_ != nullptr) {
        esp_timer_stop(volume_timer_);
        esp_timer_delete(volume_timer_);
    }
...
void LvglDisplay::ResetLowBatteryPopup() {
    DisplayLockGuard lock(this);
    low_battery_state_ = false;
...
void LvglDisplay::UpdateVolume(int volume) {
    DisplayLockGuard lock(this);
    if (volume_bar_container_ == nullptr || volume_bar_ == nullptr) {
        return;
    }

    lv_bar_set_value(volume_bar_, volume, LV_ANIM_ON);
    lv_obj_remove_flag(volume_bar_container_, LV_OBJ_FLAG_HIDDEN);
    
    lv_obj_move_foreground(volume_bar_container_);

    if (volume == 0) {
        lv_label_set_text(volume_icon_label_, FONT_AWESOME_VOLUME_XMARK);
    } else if (volume < 50) {
        lv_label_set_text(volume_icon_label_, FONT_AWESOME_VOLUME_LOW);
    } else {
        lv_label_set_text(volume_icon_label_, FONT_AWESOME_VOLUME_HIGH);
    }

    esp_timer_stop(volume_timer_);
    ESP_ERROR_CHECK(esp_timer_start_once(volume_timer_, 3 * 1000000));
}
...
void LvglDisplay::UpdateStatusBar(bool update_all) {
    ...
    if (board.GetBatteryLevel(battery_level, charging, discharging)) {
        ...
        if (low_battery_popup_ != nullptr) {
            // 低电量：5% 以下但 >0，并且在放电
            bool is_low = (battery_level > 0 && battery_level < 5 && discharging);
            if (is_low && !low_battery_state_) {
                // 从“非低电量”进入“低电量”，弹一次提示，约 10 秒后自动隐藏
                low_battery_state_ = true;
                ...
                if (low_battery_timer_ != nullptr) {
                    esp_timer_stop(low_battery_timer_);
                    ESP_ERROR_CHECK(esp_timer_start_once(low_battery_timer_, 10 * 1000000));
                }
            } else if (!is_low) {
                // 退出低电量区间，允许下次再次触发
                low_battery_state_ = false;
